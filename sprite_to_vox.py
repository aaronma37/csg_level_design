...
def convert_sprite_to_vox(image_path, output_path="character_sprite.vox", cell_size=64, index=0):
    ...
    sprite = img[y_start:y_start+cell_size, x_start:x_start+cell_size]
    
    # Use global palette
    model = VoxelModel() # Uses palette.PALETTE_COLORS by default
    
    for y in range(cell_size):
        for x in range(cell_size):
            if y >= sprite.shape[0] or x >= sprite.shape[1]: continue
            ...
            c_idx = get_nearest_character_color(rgb)
